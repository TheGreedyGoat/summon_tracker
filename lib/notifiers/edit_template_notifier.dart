...
/// Provider for values while editing a [SummonTemplate]
...
class EditTemplateState with _$EditTemplateState {
  ...
  String? validateVariables() {
    ...
    String? answer;
    if (unset.isNotEmpty) {
      answer = 'following variables are unset:\n$unset\n';
    }
    if (unnamed.isNotEmpty) {
      answer ??= '';
      answer += 'following variables are unnamed:\n$unnamed';
    }
    return answer;
  }
...
/// Notifier for values while editing a [SummonTemplate]
class EditTemplateNotifier extends Notifier<EditTemplateState> {
  @override
  EditTemplateState build() => blank;

  /// The initial state when a new Summon is created
  EditTemplateState get blank => EditTemplateState(
    ...
    skillProficiencies: CoreSkill.values
        ...
        .toList(),
    featureAbilities: [
      ActionFeature(name: 'sunlight sens', description: '...'),
    ],
    actions: [
      ActionFeature(name: 'Scimitar', description: 'Melee Weapon Attack....'),
    ],
    bonusActions: [],
    reactions: [],
    variableState: {},
    variableNames: {'[X]': 'Hans-Peter'},
  );
  ...
  ///
  @override
  set state(EditTemplateState newState) {
